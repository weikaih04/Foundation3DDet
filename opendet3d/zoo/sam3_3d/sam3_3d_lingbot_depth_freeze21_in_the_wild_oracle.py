"""SAM3_3D with LingBot-Depth on InTheWild - Oracle evaluation.

Evaluates SAM3_3D + LingBot-Depth (freeze21, 12e) on the InTheWild benchmark.

Mode: GT 2D boxes as geometry prompts (oracle), monocular depth (no GT depth).
...
def get_config() -> ExperimentConfig:
    """Returns SAM3_3D + LingBot oracle eval config for InTheWild."""
    config = get_default_cfg(
        exp_name="sam3_3d_lingbot_depth_freeze21_in_the_wild_oracle"
    )
    ...
    annotation_path = os.path.join(
        in_the_wild_data_root,
        "annotations/InTheWild_val_human_filtered.json",
    )
