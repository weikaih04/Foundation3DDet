"""SAM3_3D with LingBot-Depth on Omni3D + CubifyAnything (CA-1M).
...
Dataset mixing:
- Omni3D: ~100K images (6 datasets, train+val splits)
- CA-1M: ~206K images (CubifyAnything_train)
- dataset_ratios=[1.0, 1.0] -> equal contribution per epoch
"""
...
def get_config() -> ExperimentConfig:
    """Returns the SAM3_3D + LingBot-Depth (freeze 21) + CA-1M config."""
    ######################################################
    ##                    General Config                ##
    ######################################################
    config = get_default_cfg(
        exp_name="sam3_3d_lingbot_depth_freeze21_omni3d_ca1m"
    )
    ...
    # --- Build data config ---
    # Combine Omni3D + CA-1M as a single DataPipe, then use
    # build_train_dataloader_with_ratios for ratio-controlled sampling.
    #
    # omni3d_train_data_cfg is a DataPipe config (ConcatDataset of 12
    # Omni3D sub-datasets). ca1m_train_data_cfg is also a DataPipe config.
    # Wrap both into a new DataPipe so ConcatDataset has exactly 2
    # sub-datasets for ratio control.
    combined_train_datasets = class_config(
        DataPipe,
        datasets=[omni3d_train_data_cfg, ca1m_train_data_cfg],
    )
    ...
    # Build DataConfig directly (not via get_sam3_3d_data_cfg) to use
    # the ratio sampler for the train dataloader.
    data = DataConfig()

    data.train_dataloader = class_config(
        build_train_dataloader_with_ratios,
        dataset=combined_train_datasets,
        dataset_ratios=[1.0, 1.0],
    ...
    if isinstance(test_datasets_cfg, list):
        test_datasets_pipe = class_config(
            DataPipe, datasets=test_datasets_cfg
        )
    else:
        test_datasets_pipe = test_datasets_cfg
