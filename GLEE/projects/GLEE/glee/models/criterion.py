...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
        ...
        self.no_mask_tasks = ['obj365', 'obj365_clip', 'bdd_det', 'bdd_track_box','openimage','vg','grit', 'openimage_clip', 'glee3d']
        ...
        self.num_classes={'coco':80, 'coconomask':80, 'coco_clip':80, 'obj365':100, 'obj365_clip':100, 'lvis':100, 'lvis_clip':100, 'openimage':100, \
            'openimage_clip':100, 'grit':100, 'vg':200, 'grounding':1,  'ytbvos':1, 'rvos':1, 'sa1b':1,'sa1b_clip':1, 'ytvis19':40, 'image_yt19':40, 'ytvis21':40,\
                 ...
                 'glee3d': -1}  # glee3d: use dynamic num_classes from pred_logits shape
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, task, log=True):
        ...
        # For dynamic-vocabulary tasks (glee3d), use pred_logits dim as background index
        if num_classes < 0:
        ...
        # Ignore suppress: for glee3d, mask out negative cls loss for
        # predictions overlapping ignore boxes (like SAM3_3D)
        ignore_neg_mask = outputs.get('ignore_neg_mask')  # (B, Q) or None
        if ignore_neg_mask is not None:
            # Compute focal loss per-element (don't reduce yet)
            prob = src_logits.sigmoid()
            ce_loss = F.binary_cross_entropy_with_logits(
                src_logits, target_classes_onehot, reduction="none"
            )
            p_t = prob * target_classes_onehot + (1 - prob) * (1 - target_classes_onehot)
            focal = ce_loss * ((1 - p_t) ** 2)
            if self.focal_alpha >= 0:
                alpha_t = (self.focal_alpha * target_classes_onehot
                           + (1 - self.focal_alpha) * (1 - target_classes_onehot))
                focal = alpha_t * focal
            # Suppress: zero out loss for negative queries overlapping ignore boxes
            # ignore_neg_mask: (B, Q) float, 1=suppress, 0=normal
            # Only suppress negatives (background queries, not matched positives)
            is_negative = (target_classes == num_classes)  # (B, Q) bool
            suppress = (ignore_neg_mask > 0) & is_negative  # (B, Q) bool
            focal[suppress] = 0.0
            loss_ce = focal.mean(1).sum() / max(num_boxes, 1) * src_logits.shape[1]
        else:
    ...
    def forward(self, outputs, targets, mask_dict=None, task=None):
        ...
        if self.dn != "no" and mask_dict is not None:
            ...
            l_dict = dict()
            _device = outputs['pred_logits'].device
            l_dict['loss_bbox_dn'] = torch.as_tensor(0.).to(_device)
            l_dict['loss_giou_dn'] = torch.as_tensor(0.).to(_device)
            l_dict['loss_ce_dn'] = torch.as_tensor(0.).to(_device)
            if self.dn == "seg":
                l_dict['loss_mask_dn'] = torch.as_tensor(0.).to(_device)
                l_dict['loss_dice_dn'] = torch.as_tensor(0.).to(_device)
            losses.update(l_dict)
