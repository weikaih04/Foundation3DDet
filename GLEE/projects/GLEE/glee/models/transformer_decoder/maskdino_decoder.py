...
class MaskDINODecoder(nn.Module):
    ...
    def __init__(
        ...
        self.label_enc = {
            ...
            'glee3d':self.grounding_label_enc,
            }
    ...
    def prepare_for_dn(self, targets, tgt, refpoint_emb, batch_size,task):
        ...
        if self.training:
            ...
            # For dynamic-vocabulary tasks (glee3d), clamp labels to label_enc range
            # since contrastive classification doesn't use fixed label embeddings
            max_label_idx = self.label_enc[task].weight.shape[0] - 1
            if labels.numel() > 0:
                labels = labels.clamp(max=max_label_idx)
    ...
    def forward(self, x, mask_features, extra, task, masks, targets=None):
        ...
        if mask_dict is not None:
            ...
            # Split hs into matching and DN parts for external use (e.g. 3D head)
            hs_matching = [h[:, mask_dict['pad_size']:, :] for h in hs]
            hs_dn = [h[:, :mask_dict['pad_size'], :] for h in hs]
        else:
            track_embed =  hs[-1]
            hs_matching = list(hs)
            hs_dn = None
