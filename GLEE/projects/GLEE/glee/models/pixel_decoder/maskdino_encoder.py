...
class MaskDINOEncoder(nn.Module):
    ...
    def forward_features(self, features, masks, early_fusion=None, depth_fusion=None):
        """
        :param features: multi-scale features from the backbone
        :param masks: image mask
        :param depth_fusion: optional tuple (fusion_fn, depth_latents, depth_latents_hw)
            for injecting depth features after input_proj, before transformer encoder.
        ...
        # Depth fusion: after input_proj (256-dim), before transformer encoder
        # Analogous to SAM3_3D's depth fusion after backbone FPN, before encoder
        if depth_fusion is not None:
            fusion_fn, depth_latents, depth_latents_hw = depth_fusion
            srcs = fusion_fn(
                visual_feats=srcs,
