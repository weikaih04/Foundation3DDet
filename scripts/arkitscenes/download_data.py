"""ARKitScenes Dataset Download Script."""
...
def download_laser_scanner_point_clouds_for_video(
    ...
    for point_cloud_id in laser_scanner_point_clouds_ids:
        download_laser_scanner_point_clouds(
            point_cloud_id, visit_id, download_dir
        )
...
def download_laser_scanner_point_clouds(
    ...
    for extension in [".ply", "_pose.txt"]:
        ...
        filepath = os.path.join(
            laser_scanner_point_clouds_folder_path, filename
        )
        if os.path.exists(filepath):
            return
        file_url = f"{ARkitscense_url}/raw/laser_scanner_point_clouds/{visit_id}/{filename}"
        download_file(
            file_url, filename, laser_scanner_point_clouds_folder_path
        )
...
def download_data(
    ...
    for video_id in sorted(set(video_ids)):
        ...
        if dataset == "raw":
            ...
            if not raw_dataset_assets:
                ...
                url_prefix = (
                    f"{ARkitscense_url}/raw/{split}/{video_id}" + "/{}"
                )
        ...
        for file_name in file_names:
            ...
            if not file_name.endswith(".zip") or not os.path.isdir(
                ...
                print(
                    f"WARNING: skipping download of existing zip file: {dst_path}"
                )
    ...
    if dataset == "upsampling" and VALIDATION in dataset_splits:
        ...
        url = (
            f"{ARkitscense_url}/upsampling/{VALIDATION}/{val_attributes_file}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--video_id_csv")
    ...
    parser.add_argument(
        "--download_laser_scanner_point_cloud", action="store_true"
    )
    ...
    if args.video_id is not None:
        ...
        video_ids_ = list(
            map(str, video_ids_)
        )  # Expecting video id to be a string
